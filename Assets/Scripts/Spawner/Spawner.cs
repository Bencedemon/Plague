...
public class Spawner : NetworkBehaviour
{
    ...
    void Start()
    {
        ...
        enemySpawner.SetEnemyCount(CalculateEnemyCount(spawner_Groups.enemyCount, playerManager.PlayerGameObject.Count));
    ...
    void FixedUpdate()
    {
        ...
        if(enemyCount>=CalculateEnemyCount(spawner_Groups.enemyCount, playerManager.PlayerGameObject.Count)){
            Destroy(gameObject);   
        }
    }
    ...
    {
        while(enemyCount<CalculateEnemyCount(spawner_Groups.enemyCount, playerManager.PlayerGameObject.Count)){
    ...
    private int CalculateEnemyCount(int _enemyCount, int _playerCount){
        switch (_playerCount)
        {
            case 1:
                return _enemyCount;
            case 2:
                return (int)(_enemyCount*1.35);
            case 3:
                return (int)(_enemyCount*1.75);
            case >=4:
                return (int)(_enemyCount*2.2);
            default:
                return _enemyCount;
        }
    }
